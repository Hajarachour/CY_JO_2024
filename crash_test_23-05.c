...
void create_athlete_file(const char* athlete_name) { // Créer un fichier
    char file_path[256];
...
void add_performance(const char* athlete_name, const char* date, const char* event, const char* time, int position) { // Rajout d'une épreuve 
    char file_path[256];
    snprintf(file_path, sizeof(file_path), "%s/%s.txt", DATA_DIR, athlete_name);
...
void create_initial_athletes() { // Rajout d'athlètes 
    create_data_dir();

    // Create files and add performances
    create_athlete_file("Sanem");
    add_performance("Sanem", "2024-03-23", "4x400m", "188", 1);

    create_athlete_file("Dimittri");
    add_performance("Dimitri", "2024-04-13", "400m", "234", 0);
...
int main() {
    ...
    while (1) {
        ...
        // Create a file for the new athlete if it does not exist
        char file_path[256];
        snprintf(file_path, sizeof(file_path), "%s/%s.txt", DATA_DIR, athlete_name);
        FILE *file = fopen(file_path, "r");
        if (file == NULL) {
    ...
    // Free allocated memory (should never be reached)
    free(athlete.performances);
